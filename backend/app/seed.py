...
def _migrate_add_mbbs_other_state_access() -> None:
    """Add module_access.mbbs_other_state on existing databases (idempotent)."""
    from sqlalchemy import inspect, text
    ...
    if "module_access" not in insp.get_table_names():
        return
    cols = {c["name"] for c in insp.get_columns("module_access")}
    if "mbbs_other_state" in cols:
        return
    with engine.begin() as conn:
        conn.execute(text("ALTER TABLE module_access ADD COLUMN mbbs_other_state BOOLEAN DEFAULT FALSE"))
